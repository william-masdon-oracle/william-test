create or replace PACKAGE       "WMS_PKG_ORDS" as
procedure authorizeORDS(p_content_type varchar2 default 'application/json');
procedure authorizeEventORDS;
procedure updateWorkshops;
procedure updateWorkshop(p_ll_id number);
function updateTags(p_workshop_id number, p_ll_id number) return boolean;
procedure updateTenancies;
procedure updateEventCodes;
procedure updateIcons;
procedure getEventCodes;
procedure pushEventCodes;
procedure createGreenButton;
procedure createTag(p_tag_id number, p_level1 varchar2, p_level2 varchar2, p_level3 varchar2);
procedure getEventAttends;
procedure putEventAttends;
procedure import_image(p_tenancy_id number, p_compartment_ocid varchar2, p_image_name varchar2, p_uri varchar2, response out varchar2);
procedure createImage(p_image_id number);
procedure updateImage(p_image_id number);
procedure deleteImage(p_image_id number);
procedure createImageConnection(p_id number);
procedure updateImageConnection(p_id number, p_image_id number);
procedure deleteImageConnection(p_id number);
procedure createListing(p_listing_id number);
procedure updateListing(p_listing_id number);
procedure deleteListing(p_listing_id number);
procedure pushImages;
function getImageConnections(p_image_id number) return varchar2;
function getListingConnections(p_listing_id number) return varchar2;
procedure updateAnalytics;
function createCdnLink(p_url varchar2, p_check_exists varchar2 default 'N', p_is_event varchar2 default 'N') return varchar2;
procedure updateLiveLabsInfo(p_ll_id number);
procedure updateLiveLabsInfoAll;
procedure createAsset(p_asset_id number);
procedure updateAsset(p_asset_id number);
procedure deleteAsset(p_asset_id number);
procedure createAssetType(p_type_id number);
procedure updateAssetType(p_type_id number);
procedure deleteAssetType(p_type_id number);
procedure createLiveStack(p_livestack_id number);
procedure updateLiveStack(p_livestack_id number);
procedure deleteLiveStack(p_livestack_id number);
procedure createLiveStackAsset(p_livestack_id number, p_asset_id number);
procedure updateLiveStackAsset(p_livestack_id number, p_asset_id number);
procedure deleteLiveStackAsset(p_livestack_id number, p_asset_id number);
procedure createLiveStackEntry(p_entry_id number);
procedure updateLiveStackEntry(p_entry_id number);
procedure deleteLiveStackEntry(p_entry_id number);
procedure createLLType(p_type_id number);
procedure updateLLType(p_type_id number);
procedure deleteLLType(p_type_id number);
end;
/
    ...
    begin
        ...
        select
            ...
            json_object (
                ...
                'wms_id' value w.id,
                'lab_url' value '' ||  
                    case when nvl(ll.github_override_flg, 'N') = 'Y' then
                        json_object ( 
                        ...
                        json_object ( 
                            'FREETIER_URL' value case when ll.freetier_url is not null then WMS_PKG_ORDS.createCDNLink(ll.freetier_url) || v_mrm_url else null end, 
                            'ALWAYSFREE_URL' value case when ll.alwaysfree_url is not null then WMS_PKG_ORDS.createCDNLink(ll.alwaysfree_url) || v_mrm_url else null end, 
                            'PAID_URL' value case ll.sprint_flg when 'Y' then case when ll.sprint_url is not null then WMS_PKG_ORDS.createCDNLink(ll.sprint_url) || v_mrm_url else null end else case when ll.paid_url is not null then WMS_PKG_ORDS.createCDNLink(ll.paid_url) || v_mrm_url else null end end, 
                            'LIVELABS_URL' value case when ll.greenbutton_url is not null then WMS_PKG_ORDS.createCDNLink(ll.greenbutton_url) || v_mrm_url else null end, 
                            'DESKTOP_GUIDE_URL' value ll.desktopguide_url, 
                            'DESKTOP_APP1_URL' value ll.desktopapp1_url, 
                            'DESKTOP_APP2_URL' value ll.desktopapp2_url 
                        ) || ''
...
begin
    ...
    v_url := trim(REGEXP_REPLACE(replace(p_url,'oracle-livelabs.github.io', 'livelabs.oracle.com/cdn'), '\?.*$', ''));
    v_url := regexp_replace(v_url, '/+$', '');

    if regexp_like(v_url, '/index\.html$', 'i') then
        null;
    elsif regexp_like(v_url, '/index$', 'i') then
        v_url := v_url || '.html';
    else
        v_url := v_url || '/index.html';
    end if;
    ...
    for event in c_events loop
        ...
        begin
            ...
            if apex_web_service.g_status_code between 200 and 299 then
                ...
                elsif (event.updated_flg = 2) and v_response is null then
                    insert into ords_log(priority, workshop_id, livelabs_id, error_message)
